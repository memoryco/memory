...
**Storage:** Embeddings are stored as raw bytes in the `memorys` SQLite table (`embedding BLOB`). Conversion between `Vec<f32>` and bytes uses `embedding_to_bytes`/`bytes_to_embedding` from `src/memory/storage/models.rs`.
...
**Search execution** (`src/memory/storage/vector.rs` — `VectorSearch::find_similar`):

1. Loads **all** memorys with non-null embeddings from SQLite in a single query
...
There is no approximate nearest neighbor (ANN) index — this is an exhaustive scan. This is fine for the current scale (hundreds to low thousands of memorys) but would need an index (e.g., HNSW, IVF) at much larger scale.

### Score Blending & Ranking

The MCP tool `memory_search` (`src/tools/memory/search.rs`) adds a **blended scoring layer** on top of raw vector similarity:
...
1. Looks up each result's in-memory `Memory` via `brain.get_or_load()` (handles cross-process writes)
...
Recall (`src/memory/substrate.rs` — `Substrate::recall`) is the "I'm using this memory" operation. Unlike search, it has side effects:

**Stimulation chain:**
1. **Energy boost:** Adds `recall_strength` (default 0.2) to the memory's energy, capped at 1.0
...
4. **Resurrection:** If the memory was in `Deep` or `Archived` state and is now `Active`/`Dormant`, it's "resurrected" — returned to normal search results
5. **Hebbian learning:** Strengthens/creates associations with recently recalled memorys (see below)
...
**Association structure** (`src/memory/association.rs`):
- `from: MemoryId` — source
- `to: MemoryId` — target
...
1. **Hebbian learning (automatic):** When an memory is stimulated, the system checks the last 5 recalled memorys (`recent_window = 5`). For each recent memory that is still searchable, bidirectional associations are created or strengthened:
...
2. **Explicit association** (`brain.associate(from, to, weight)`): Manual creation via the `memory_associate` MCP tool. Weight is user-specified.
...
**Association decay:** Associations decay alongside memorys. During `apply_time_decay`:
```
...
- Forward: `HashMap<MemoryId, Vec<Association>>` — keyed by source
- Reverse: `HashMap<MemoryId, Vec<MemoryId>>` — "who points to me" index

### Energy Decay & State Transitions

**Memory states** and their energy thresholds (`src/memory/memory.rs`):
...
4. Applies decay to every memory (except Archived, which is frozen, and Deep, which decays at 25% rate)
5. Applies proportional decay to all associations
6. Energy never reaches true zero — floor is 0.001

**Lazy execution:** Decay is applied lazily on the next `memory_search` or `memory_recall` call (via `brain.apply_time_decay()`), not on a timer. This means decay catches up retroactively based on elapsed wall-clock time.
...
**`src/memory/convergence.rs`** — Core convergence algorithm, operating on `Substrate` + `Storage`:
...
pub struct ConvergenceHit {
    ...
    pub discovered_via: Option<(MemoryId, f64)>,  // (source_id, assoc_weight)
...
**`src/tools/memory/converge.rs`** — MCP tool wrapper that:
...
- `brain.get_or_load()` — for memory metadata (energy, state)
...
- **Deep memories:** Only participate if `include_deep` is true. Their associations are still followed during expansion (if they happen to be in the association graph), but Deep memorys themselves are only added to results if the flag is set
- **Archived memories:** Same as Deep but with `include_archived`
- **Convergence search is passive** (like `memory_search`): It does NOT stimulate memories. The caller should use `memory_recall` on results they actually use. This preserves the clean separation between search (read-only observation) and recall (active reinforcement)
...
```rust
fn get_embeddings_batch(&mut self, ids: &[MemoryId]) -> StorageResult<HashMap<MemoryId, Vec<f32>>>;
```
...
│       └── mod.rs              # Register MemoryConvergeTool
└── ...
```

### Edge Cases & Considerations

**1. Circular associations:** A→B→C→A would cause infinite loops without the `seen_ids` set. The algorithm tracks all visited IDs across all passes, so each memory is only scored once.
