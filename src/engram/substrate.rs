...
impl Substrate {
    ...
    pub fn stimulate(&mut self, id: EngramId, amount: f64) -> StimulationResult {
        ...
        if self.get(&id).map(|e| e.state.can_propagate()).unwrap_or(false) {
    ...
    fn apply_hebbian_learning(&mut self, current_id: EngramId) -> Vec<(EngramId, EngramId)> {
        ...
        for recent_id in recent {
            if recent_id != current_id {
                ...
                let recent_searchable = self.get(&recent_id)
    ...
    pub fn list_tags(&self) -> Vec<String> {
        let mut tags: Vec<String> = self.engrams.values()
    ...
    pub fn search_with_options(&self, query: &str, options: SearchOptions) -> Vec<&Engram> {
        ...
        let mut results: Vec<&Engram> = self.engrams.values()
            .filter(|e| {
                // State filter
        ...
        if options.sort_by_energy {
            results.sort_by(|a, b| b.energy.partial_cmp(&a.energy).unwrap_or(std::cmp::Ordering::Equal));
        }
    ...
    pub fn search_by_tags_with_options(&self, tags: &[&str], mode: TagMatchMode, options: SearchOptions) -> Vec<&Engram> {
    ...
    pub fn stats(&self) -> SubstrateStats {
        ...
        let association_count: usize = self.associations.values()
            .map(|v| v.len())
            .sum();
...
mod tests {
    ...
    fn build_test_substrate() -> Substrate {
        ...
        substrate.create_with_tags("Rust FFI patterns discussion", vec!["work".into(), "rust".into()]);
        substrate.create_with_tags("Swift concurrency debugging", vec!["work".into(), "swift".into()]);
        ...
        substrate.create_with_tags("TwilioDataCore architecture", vec!["work".into(), "rust".into()]);
    ...
    fn search_respects_state_filter() {
        ...
        let all_results = substrate.search_by_tag_with_options(
            "personal", 
            SearchOptions::default().include_all()
        );
    ...
    fn search_with_limit() {
        ...
        let results = substrate.search_by_tag_with_options(
            "work",
            SearchOptions::default().with_limit(2)
        );
