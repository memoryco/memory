...
use super::{IdentityStorage, IdentityItemType, IdentityItemRow, NewIdentityItem, StorageResult, StorageError};
...
impl IdentityStorage for DieselIdentityStorage {
    ...
    fn list_items(&mut self, item_type: Option<IdentityItemType>) -> StorageResult<Vec<IdentityItemRow>> {
...
mod tests {
    ...
    fn list_items_by_type() {
        ...
        storage.add_item(IdentityItemType::Expertise, "Rust", None, None, None).unwrap();
        storage.add_item(IdentityItemType::Expertise, "Python", None, None, None).unwrap();
        storage.add_item(IdentityItemType::Trait, "curious", None, None, None).unwrap();
        
        let expertise = storage.list_items(Some(IdentityItemType::Expertise)).unwrap();
    ...
    fn remove_item() {
        ...
        let id = storage.add_item(IdentityItemType::Expertise, "Rust", None, None, None).unwrap();
    ...
    fn persona_fields() {
        ...
        storage.add_item(IdentityItemType::PersonaName, "Porter", None, None, None).unwrap();
        storage.add_item(IdentityItemType::PersonaDescription, "A software engineer who...", None, None, None).unwrap();
        
        let name = storage.list_items(Some(IdentityItemType::PersonaName)).unwrap();
        assert_eq!(name.len(), 1);
        assert_eq!(name[0].content, "Porter");
        
        let desc = storage.list_items(Some(IdentityItemType::PersonaDescription)).unwrap();
