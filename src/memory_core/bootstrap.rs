...
2. Search for context → Call `memory_search` with keywords from the user's message
...
3. Recall what you'll use → Call `memory_recall` with IDs of memories you will reference
...
6. Store what you learned → Call `memory_create` with every new fact from this turn
   This step is MANDATORY, not optional. If you learned anything new — from the user's message, from reading files, from reaching conclusions — call `memory_create` before finishing your response. Facts you don't store are permanently lost; there is no second chance. Batch multiple atomic memories into a single call using the memories array parameter.

   For long turns with many tool calls, create incrementally: batch ~3–5 facts into an `memory_create` call as you discover them, then continue working. This guards against conversation interruption and ensures earlier memories get wired to later ones via session tracking. But even on short turns, always do a final check: did I learn anything new? If yes, store it.
</workflow>
...
If results include a 🔗 procedure chain hint, call `memory_associations` on the anchor with direction: outbound to get the full ordered steps before proceeding.
</searching_memories>

<recalling_memories>
Call `memory_recall` IMMEDIATELY after search, BEFORE writing your response. Review the search results, identify which memories you will reference, and recall them right away while tool budget is available.
...
- Does it use a date as a prefix or label? Remove it — `created_at` captures this automatically, and `memory_search` supports `created_after`/`created_before` params for time-based filtering. Only keep dates that ARE the fact (deadlines, birthdays, scheduled events).
...
Call `memory_create` at least once per turn if you learned anything new. This is not optional — skipping creation means permanent information loss.

On long turns with many tool calls, create incrementally in batches of ~3–5 facts as you discover them. This applies especially when reading code, exploring filesystems, or processing multi-step user input. On short turns, a single `memory_create` call before finishing your response is sufficient.

Self-check before responding: "Did I learn any new facts this turn that aren't already in memory?" If the answer is yes and you haven't called `memory_create`, stop and do it now.
...
3. Wire anchor → each step using `memory_associate` with ordinals 1–N and weight 0.8

This makes the procedure discoverable via search and walkable via `memory_associations`.
...
1. Delete the anchor memory AND all its step memories using `memory_delete`
   - Deleting a memory automatically removes all associations from/to it
2. Recreate the anchor and all steps fresh with `memory_create`
3. Rewire the chain with `memory_associate` using correct ordinals 1–N
4. Verify with `memory_associations` (direction: outbound) on the new anchor
