...
use super::{PlanStorage, StorageResult, StorageError};
...
impl PlanStorage for DieselPlanStorage {
    ...
    fn delete_plan(&mut self, id: &PlanId) -> StorageResult<bool> {
        ...
        diesel::delete(steps::table.filter(steps::plan_id.eq(&id_str)))
            .execute(&mut self.conn)?;
        
        let deleted = diesel::delete(plans::table.filter(plans::id.eq(&id_str)))
            .execute(&mut self.conn)?;
